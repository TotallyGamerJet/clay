...
type RendererData struct {
	...
	// Copies of Fonts at other sizes. A *ttf.Font is SDL's own pointer rather than
	// memory Go manages, so SDL can reuse it for another font once one is closed. The
	// copies are only valid for as long as the fonts they were made from, which is why
	// they are kept here rather than for the whole program.
