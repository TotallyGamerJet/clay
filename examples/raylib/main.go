...
func main() {
	...
	arena := clay.CreateArenaWithCapacityAndMemory(uint64(totalMemorySize), unsafe.Pointer(unsafe.SliceData(make([]byte, totalMemorySize))))
	clay.Initialize(arena, clay.Dimensions{Width: float32(rl.GetScreenWidth()), Height: float32(rl.GetScreenHeight())}, clay.ErrorHandler{ErrorHandlerFunction: handleClayError})
	raylib.Initialize(1024, 768, "Clay - Raylib Renderer Example", rl.FlagVsyncHint|rl.FlagWindowResizable|rl.FlagWindowHighdpi|rl.FlagMsaa4xHint)
...
func updateDrawFrame(fonts []rl.Font) {
	...
	raylib.Render(renderCommands, fonts)
}
